    ...
    if (validuser) {
      res
        ...
        .send({
          name: exist.name,
          email: exist.email,
          wishlist: exist.wishlist,
          bookedlist: exist.bookedlist,
        });
    } else {
...
app.get("/category/:q", async (req, res) => {
  ...
  if (filter > 0) {
    ...
    const places = await PlaceModel.find({ category: q }).lean();
...
app.get("/search/:q", async (req, res) => {
  ...
  if (filter > 0) {
    if (filter <= 100000) {
      const search_by_city = await PlaceModel.find({
        city: { $regex: q, $options: "i" },
        price: { $lte: filter },
      });
      if (q !== "NA") {
        var state_one = await PlaceModel.find({
          state: { $regex: q, $options: "i" },
          price: { $lte: filter },
        });
      }
      const search_by_country = await PlaceModel.find({
      ...
      if (q !== "NA") {
        var state_two = await PlaceModel.find({
          state: { $regex: q, $options: "i" },
          price: { $gt: filter },
        });
      }
    ...
    if (q !== "NA") {
      var state_three = await PlaceModel.find({
        state: { $regex: q, $options: "i" },
      });
    }
    const search_by_country = await PlaceModel.find({
...
app.get("/get-wish-list/:q", async (req, res) => {
  const { q } = req.params;
  const response = await UserModel.findOne({ email: q })
    .populate("wishlist")
    .exec();
  res.send(response);
});
app.put("/wish-list/:q/:id", async (req, res) => {
  ...
  if (!curr.includes(id)) {
    ...
    const v = await UserModel.findOneAndUpdate(
      { email: q },
      { wishlist: [...updated] },
      { new: true }
    );
...
app.post("/booking/:email/:id", async (req, res) => {
  ...
  const start = `${st.date > 9 ? st.date : `0${st.date}`}-${
    st.month > 8 ? st.month + 1 : `0${st.month + 1}`
  }-${st.year}`.toString();
  const end = `${en.date > 9 ? en.date : `0${en.date}`}-${
    en.month > 8 ? en.month + 1 : `0${en.month + 1}`
  }-${en.year}`.toString();
...
app.get("/booked-places/:email", async (req, res) => {
  ...
  for (let p of user.bookedlist) {
    const b = await BookingModel.findOne({ _id: p._id })
      .populate("booked")
      .exec();
    bookedplaces.push(b);
  }
